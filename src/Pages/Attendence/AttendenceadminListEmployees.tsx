...
function AttendenceuserListEmployees() {
      ...
      {!isLoading && (
        <div className="table p-5">
          <div className="d-flex align-items-center justify-content-between">
            <h1 className="text-success">Attendence Record: {data && data.result && data.result.length > 0 ? data.result.find((attendence: attendenceModel) => attendence.employeeId === userId)?.name : 'Employee Name Not Found'}</h1>
  ...
  data.result
    ...
    .map((attendence: attendenceModel, index: number) => (
                <div className="row border" key={attendence.id || index}>
                  ...
                  <div className="col-1">
                    {new Date(attendence.workingDate).toLocaleDateString()}
                  </div>
                  <div className="col-2">
                    {new Date(attendence.clockInTime).toLocaleTimeString()}
                  </div>
                  <div className="col-2">
                    {new Date(attendence.clockOutTime).toLocaleTimeString()}
                  </div>
...
export default AttendenceuserListEmployees;
